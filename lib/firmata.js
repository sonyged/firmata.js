...
MIDI_RESPONSE[DIGITAL_MESSAGE] = function(board) {
  ...
  for (var i = 0; i < 8; i++) {
    ...
    if (pin && (pin.mode === board.MODES.INPUT || pin.mode === board.MODES.INPUT_PULLUP)) {
...
var KOOV_OP = 0x0e;
SYSEX_RESPONSE[KOOV_OP] = function(board) {
  var type = board.currentBuffer[2];

  // console.log(`sysex: ${type}`);
  if (type === 0x01) {		// accel.
    var direction = board.currentBuffer[3];
    var v0 = board.currentBuffer[4];
    var v1 = board.currentBuffer[5];
    //console.log(`sysex: 0x01: direction ${direction} ${v0} ${v1}`);

    var value = v0 | (((v1 << (32 - 7)) >> (32 - 7)) << 7);
    board.emit("accelerometer-read", {
      direction: direction,
      value: value
    });
    return;
  ...
  if (type === 0x02) {		// KOOV generic extenstion
    var command = board.currentBuffer[3];
    if (command === 0x01) {
      board.emit("bts01-reset", {
	buffer: board.currentBuffer.slice(4, board.currentBuffer.length - 1)
      });
    }
    if (command === 0x02) {
      board.emit("bts01-cmd", {
	buffer: board.currentBuffer.slice(4, board.currentBuffer.length - 1)
      });
    }
    if (command === 0x08) {
      board.emit("flash-erase", {
	buffer: board.currentBuffer.slice(4, board.currentBuffer.length - 1)
      });
    }
    if (command === 0x09) {
      board.emit("flash-finish", {
	buffer: board.currentBuffer.slice(4, board.currentBuffer.length - 1)
      });
    }
    if (command === 0x0a) {
      board.emit("flash-write", {
	buffer: board.currentBuffer.slice(4, board.currentBuffer.length - 1)
      });
    }
    if (command === 0x0b) {
      var subcmd = board.currentBuffer[4];
      if (subcmd === 0x03) {
        if (board.currentBuffer[5] === 0) {
          var firmwareBuf = [];
          board.firmware.version = {};
          board.firmware.version.major = board.currentBuffer[6];
          board.firmware.version.minor = board.currentBuffer[7];
          for (var i = 8, length = board.currentBuffer.length - 2; i < length; i += 2) {
            firmwareBuf.push((board.currentBuffer[i] & 0x7F) | ((board.currentBuffer[i + 1] & 0x7F) << 7));
          }
          ...
          board.firmware.version = {};
          board.firmware.name = null;
        }
        board.emit("pinchallenge");
      } else {
        board.emit("btpin", {
	  buffer: board.currentBuffer.slice(4, board.currentBuffer.length - 1)
        });
    ...
    if (command === 0x0d) {
      board.emit("led-matrix", {
	buffer: board.currentBuffer.slice(4, board.currentBuffer.length - 1)
      });
    }
    if (command === 0x0e) {
      board.emit("ultrasonic-distance-sensor", {
	buffer: board.currentBuffer.slice(4, board.currentBuffer.length - 1)
      });
    }
    if (command === 0x0f) {
      board.emit("color-sensor", {
	buffer: board.currentBuffer.slice(4, board.currentBuffer.length - 1)
      });
    }
  }
...
var Board = function(port, options, callback) {
  ...
  this.reportVersionTimeoutId = setTimeout(function() {
    if (this.versionReceived === false) {
      ...
      this.pinchallenge(options.btpin, function() {});
    }
  ...
  this.once("reportversion", function() {
    ...
    this.versionReceived = true;
    this.once("pinchallenge", function() {
...
Board.prototype.pinchallenge = function(pin, callback) {
  this.once("pinchallenge", callback);
  this.transport.write(new Buffer([
      START_SYSEX, 0x0e, 0x02, 0x0b, 0x03, pin & 0x7f, (pin >> 7) & 0x7f,
      END_SYSEX]));
};
